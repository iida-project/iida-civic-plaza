...
`organizations.is_contact_public` (boolean, デフォルト true) で連絡先（担当者名・メール・電話）の公開を制御。
- 管理フォームの「連絡先」セクションにスイッチを配置
- `false` の場合、公開サイトの連絡先カードが非表示になる
- Web・SNSセクションには影響しない

## 「サイトで確認」リンク

全編集ページ（団体・インタビュー・助成金・お知らせ）のヘッダーに ExternalLink アイコン付きの「サイトで確認」ボタンを配置。新しいタブで公開サイトの該当ページを開く。団体一覧の操作列にも同様のリンクあり。
